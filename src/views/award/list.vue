<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <enabled
            ...
            :url="'/award/' + scope.row.id"
          />
        </template>
      </el-table-column>

      <el-table-column label="Details" width="600px" align="center">
        <template slot-scope="scope">
          <el-row
            type="flex"
            justify=""
          ...
          >
            ...
            <el-col :span="16">
              ...
              <br><br>
              <b>Link:</b> <br><span><a :href="scope.row.link" target="_blank">{{ scope.row.link }}</a></span>
              <br><br>
              <b>Source:</b> <span>{{scope.row.source}}</span> 
            </el-col>
          </el-row>
        </template>
      </el-table-column>
       <!-- <el-table-column label="Source" width="200px" align="center">
        <template slot-scope="scope">
            ...
            >
             <el-tag>
              <span>{{ scope.row.source.slice(scope.row.source.length -16 - scope.row.source.length) }}</span></el-tag></a>
      ...
      >
        <template slot-scope="scope">
          <router-link :to="'/award/edit/' + scope.row.id">
            <el-button
              type="warning"
              size="small"
              icon="el-icon-edit"
      ...
      <!-- <el-table-column align="center" :label="$t('table.actions')" width="180">
        <template slot-scope="scope">
          <router-link :to="'/Award/view/'+scope.row.id">
...
import { IAwardData } from '@/api/types'
...
export default class extends Vue {
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        "Do you want to delete this awards?",
        ...
        .then(() => {
          deleteAward(data.id).then((deleted) => {
            this.getList();
          });
        })
        .catch(() => {
  ...
  private async getList() {
    try {
      ...
      const data: any = await getAward(getQuery(this.listQuery))
      this.list = data.data
  ...
  private handleDownload() {
    ...
    exportJson2Excel(tHeader, data, this.$tc('route.Award.description'))
