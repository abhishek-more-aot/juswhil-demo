        ...
        <template slot-scope="scope">
          ...
          >
            ...
            <el-col :span="16">
              ...
              <br><br>
              <b>Source:</b> <br><span>{{ scope.row.source }}</span>
            </el-col>
          </el-row>
        </template>
      </el-table-column>
...
import { INewsData } from '@/api/types'
...
export default class extends Vue {
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        ...
        .then(() => {
          deleteNews(data.id).then((deleted) => {
            this.getList();
          });
        })
        .catch(() => {
  ...
  private async getList() {
    try {
      ...
      const data: any = await getNews(getQuery(this.listQuery))
      this.list = data.data
  ...
  private handleDownload() {
    ...
    exportJson2Excel(tHeader, data, this.$tc('route.news.description'))
