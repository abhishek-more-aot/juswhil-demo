<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <router-link :to="'/trending/add'" class="margin-horizontal">
        <el-button
          v-waves
          class="filter-item"
          type="primary"
    ...
    >
      ...
      <el-table-column width="400px" align="center" prop="" label="Product Details">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('table.actions')" width="180">
        <template slot-scope="scope">
          <router-link :to="'/trending/edit/' + scope.row.id">
            <el-button
              v-waves
              type="warning"
              size="small"
...
export default class extends Vue {
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        "Do you want to delete this trending?",
        ...
        .then(() => {
          deleteTrending(data.id).then((deleted) => {
            this.getList();
          });
        })
        .catch(() => {
  ...
  private async getList() {
    try {
      this.listLoading = true;
      const data: any = await getTrending(getQuery(this.listQuery));
      this.list = data.data;
  ...
  private handleDownload() {
    ...
    exportJson2Excel(
      tHeader,
      data,
      this.$tc("route.TrendingManagement.Trending.title")
    );
